...
class Utils_Upload {
	...
	public static function factory($destination, $root = false) {
		...
		return new Utils_Upload($destination, $root);
	...
	protected function create_new_filename() {
		...
		if (isset($this->ext[$this->file['mime']])) {
			$filename .= '.'.$this->ext[$this->file['mime']];
		}
	 ...
	 * 上传合同或模板
	 *
	 * @param array $temp_info , int type
	 * @return string or bool
	 */
	public function upload_contract($temp_info,$type=1) {
            //上传模板
            if($type == 1){
                //文件名
                $file_name = trim($temp_info['oid']."_".$temp_info['type']."_".$temp_info['id'].".html");
                //文件完整路径
                $contract_template_file_path = $this->root.$this->destination.$file_name;
                //将模板内容写入服务器文件
                $byte_size = file_put_contents($contract_template_file_path, $temp_info['content']);
            }
            //上传合同
            else if($type == 2){
                //文件名
                $file_name = trim($temp_info['oid']."_".$temp_info['type']."_".$temp_info['appid'].".html");
                //文件完整路径
                $contract_file_path = $this->root.$this->destination.$file_name;
                //将模板内容写入服务器文件
            ...
            if(is_numeric($byte_size) && $byte_size > 0){
                return $this->destination.$file_name;
            }
            else{
                return false;
            }           
